...
public class Note : MonoBehaviour
{
    ...
    public void Initialize(float damage, float speed, int hit_Limit, float Slow, float SlowT, Watame watame)
    {
        this.damage = damage;
        this.speed = speed;
    ...
    void HitEnemy(RaycastHit2D hit)
    {
        ...
        if (enemyScript != null && hit_Limit > 0 && !hitEnemies.Contains(enemyScript))
        {
            hitEnemies.Add(enemyScript);
            enemyScript.UpdateAttackingTower(watame);
