...
public class LunaCandy : MonoBehaviour
{
    ...
    public void Initialize(float Power, Luna luna)
    {
                ...
                if (hit.collider != null && hit.collider.gameObject.CompareTag("Tower"))
                {
                    ...
                    if (tower != null)
                    {
                        if (!tower.isEx)
                        {
                            ...
                            if (tower.dscName == "Luna")
                            {
                                byb = false;
                                Eat.Play();
                                tower.Heal(Power, tower);
                                PlayAnimationAndSound(tower, mprefab);
                                luna.LunaTime();
                            }
                            else
                            {
                                ...
                                if (gameManager != null)
                                {
                                    foreach (var t in gameManager.AllTowers.Where(t => !t.isEx && !t.isDead && t.dscName != "Luna"))
                                    {
                                        Eat.Play();
                                        t.ApplyBuff(myName);
                                        t.Heal(Power, luna);
                                        PlayAnimationAndSound(t, oprefab);
                                    }
                                }
                            }
                            GameManager.Instance.StopM();
                            byb = false;
                        }
                    }
                }
    ...
    private void PlayAnimationAndSound(Tower tower, GameObject gameObject)
    {
        if (gameObject != null)
        {
            Instantiate(gameObject, Vector3.zero, Quaternion.identity, tower.transform);
        }

        if (gameObject == mprefab)
        {
            Instantiate(oprefab, Vector3.zero, Quaternion.identity, tower.transform);
        }
